...
class AbstractCommand:
    ...
    def get_usage(self):
        return f"  ./docsearch {self.get_name()}"

    def get_options(self):
        return []

    def nb_options(self):
        return len([x for x in self.get_options() if x.get("optional") is None])
    ...
    def get_option(self, name, args):
        ...
        return args[index] if index < len(args) else options[index]["optional"]

    def check_docsearch_app_id(self, action_description):
        if environ.get("APPLICATION_ID") != "BH4D9OD16A":
            print(
                f"The APP_ID is not BH4D9OD16A. You can not {action_description} if you are not using the docsearch account"
            )
            exit(1)
    ...
    def check_not_docsearch_app_id(self, action_description):
        if environ.get("APPLICATION_ID") == "BH4D9OD16A":
            print(
                f"The APP_ID is BH4D9OD16A. You can not {action_description} if you are using the docsearch account"
            )
            exit(1)
